...
assert sys.argv[-1].lower() in ('legendre', 'chebyshev', 'chebyshevu', 'jacobi')
...
SD = FunctionSpace(N, family=family, bc=bc, alpha=1, beta=2) # alpha, beta are ignored by all other than jacobi
u = TrialFunction(SD)
v = TestFunction(SD)
...
A0 = inner(v, -div(grad(u))) + alpha*inner(v, u)

# Solve
u_hat = Function(SD).set_boundary_dofs()
Solver = chebyshev.la.Helmholtz if family == 'chebyshev' and alpha != 0 else la.Solver
M = Solver(A0)
