...
class BCDCDmat(SpectralMatrix):
    ...
    def __init__(self, test, trial, scale=1, measure=1):
        ...
        B = BQQmat((test[0].get_orthogonal(domain=(-1, 1)), 0), (trial[0].get_orthogonal(domain=(-1, 1)), 0))
...
class BGBCGmat(SpectralMatrix):
    ...
    :class:`.ultraspherical.bases.CompositeBase`, the
    trial :math:`\psi_j \in` :class:`.ultraspherical.bases.BCGeneric`, and test and
    trial spaces have dimensions of M and N, respectively.
    ...
    def __init__(self, test, trial, scale=1, measure=1):
        ...
        q = sp.degree(measure)
        if measure != 1:
            from shenfun.jacobi.recursions import pmat, cn, a
            ...
            for dv in split(measure, expand=True):
                ...
                msi = dv['x']
                qi = sp.degree(msi)
                Ax = pmat(a, qi, test[0].alpha, test[0].alpha, test[0].N, test[0].N, cn)
                A = A + sc*Ax.diags('csr')
...
class BGGmat(SpectralMatrix):
    ...
    any subclasses of :class:`.ultraspherical.bases.CompositeBase` and :math:`q \ge 0`
...
class PXGmat(SpectralMatrix):
    ...
    where the test function :math:`\phi_i` is in one of :class:`.ultraspherical.bases.Phi1`,
    :class:`.ultraspherical.bases.Phi2`, :class:`.ultraspherical.bases.Phi3`,
    :class:`.ultraspherical.bases.Phi4`,
    the trial :math:`\psi_j` any class in :class:`.ultraspherical.bases`,
    ...
    def __init__(self, test, trial, scale=1, measure=1):
        ...
        from shenfun.jacobi.recursions import Lmat, cn
        q = sp.degree(measure)
        ...
        if q > 0 and test[0].domain != test[0].reference_domain():
            ...
            for dv in split(measure, expand=True):
                ...
                msi = dv['x']
                qi = sp.degree(msi)
                Ax = Lmat(k, qi, l, test[0].dim(), trial[0].N, test[0].alpha, test[0].alpha, cn)
                D = D + sc*Ax
        else:
            D = Lmat(k, q, l, test[0].dim(), trial[0].N, test[0].alpha, test[0].alpha, cn)
...
class PXBCGmat(SpectralMatrix):
    ...
    :class:`.ultraspherical.bases.Phi2`, :class:`.ultraspherical.bases.Phi3`,
    :class:`.ultraspherical.bases.Phi4`,
    trial :math:`\psi_j \in` :class:`.ultraspherical.bases.BCGeneric`.
    ...
    def __init__(self, test, trial, scale=1, measure=1):
        ...
        if q > 0 and test[0].domain != test[0].reference_domain():
            ...
            for dv in split(measure, expand=True):
                ...
                msi = dv['x']
                qi = sp.degree(msi)
                Ax = Lmat(k, qi, l, M, N, test[0].alpha, test[0].alpha, cn)
                D = D + sc*Ax
        else:
            D = Lmat(k, q, l, M, N, test[0].alpha, test[0].alpha, cn)
